...
if [ $flag != 0 ] ; then echo  "ERROR! Red Hat Subscription Manager Registration Failed" | log; exit $flag;  fi
...
if [ $flag != 0 ] ; then echo  "ERROR! Pool Attach for JBoss EAP Failed" log; exit $flag;  fi
...
if [ $flag != 0 ] ; then echo  "ERROR! Enabling repos for JBoss EAP Failed" | log; exit $flag;  fi
...
if [ $flag != 0 ] ; then echo  "ERROR! JBoss EAP installation Failed" | log; exit $flag;  fi
