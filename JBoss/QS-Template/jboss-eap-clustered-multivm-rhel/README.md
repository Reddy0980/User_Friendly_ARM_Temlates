...
This Azure Resource Manager (ARM) template creates all the Azure compute resources to run JBoss EAP 7.2/EAP 7.3/EAP 7.4 cluster running 'n' number of Red Hat Enterprise Linux (RHEL) 7.7/8.0/8.4 VMs, where 'n' is decided by the user and all the VMs are added to the backend pool of a Load Balancer. The following resources are created by this template:

- RHEL 7.7/8.0/8.4 Virtual Machines
- 1 Load Balancer
- Virtual Network with a single subnet
- JBoss EAP 7.2/EAP 7.3/EAP 7.4 cluster setup on the RHEL VMs
- Sample Java application called **eap-session-replication** deployed on JBoss EAP
- Storage Account

Note that the users also have the option to choose between the Red Hat Enterprise Linux versions 7.7, 8.0 and 8.4 and JBoss EAP versions 7.2, 7.3 and 7.4. Users can select one of the following combinations for deployment
...
Build your environment with JBoss EAP 7.2/EAP 7.3/EAP 7.4 cluster setup on 'n' number of RHEL 7.7/8.0/8.4 VMs where 'n' is decided by the user and all the VMs are added to the backend pool of a Load Balancer on Azure in a few simple steps:
...
`Tags: JBoss, Red Hat, EAP 7.2, EAP 7.3, EAP 7.4, Cluster, Load Balancer, RHEL 7.7, RHEL 8.0, RHEL 8.4, Azure, Azure VM, Java EE`
