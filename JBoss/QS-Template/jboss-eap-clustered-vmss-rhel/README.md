...
This Azure Resource Manager (ARM) template creates all the Azure compute resources to run JBoss EAP 7.2/EAP 7.3/EAP 7.4 cluster running Red Hat Enterprise Linux (RHEL) 7.7/8.0/8.4 VMSS instances where the user can decide the number of instances to be deployed and scale it according to their requirement. The following resources are created by this template:

- RHEL 7.7/8.0/8.4 Virtual Machine Scale Set instances
- 1 Load Balancer
- Virtual Network with a single subnet
- JBoss EAP 7.2/EAP 7.3/EAP 7.4 cluster setup on the VMSS instances
- Sample Java application called **eap-session-replication** deployed on JBoss EAP
- Storage Account

Note that the users also have the option to choose between the Red Hat Enterprise Linux versions 7.7, 8.0 and 8.4 and JBoss EAP versions 7.2, 7.3 and EAP 7.4. Users can select one of the following combinations for deployment
...
Build your environment with JBoss EAP 7.2/EAP 7.3/EAP 7.4 cluster setup on RHEL 7.7/8.0/8.4 VMSS instances where the user can decide the number of instances to be deployed and scale it according to their requirement on Azure in a few simple steps:
...
2. Complete the following parameter values and accept the Terms and Conditions before clicking on the **Purchase** button.
    ...
    - **EAP on RHEL Version** - Select the EAP on RHEL version combination from the dropdown options, the default option selected here is JBoss EAP 7.4 on RHEL 8.4.
...
`Tags: JBoss, Red Hat, EAP 7.2, EAP 7.3, EAP 7.4, Cluster, Load Balancer, RHEL 7.7, RHEL 8.0, RHEL 8.4, Azure, Azure VMSS, Java EE`
